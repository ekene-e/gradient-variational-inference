...
def get_HESS_h2_SS(XtX, XX, LD, beta, se, N, var_Y, LDthres=0.1):
    ...
    while len(idx_exclude) > 0:
        ...
        idx_exclude = [i for i in idx_exclude if i not in np.where(
            LD[maxid, :] > LDthres)[0]]
    ...
    h2_hess = (np.dot(np.dot(vec_id.transpose(), R_inv),
...
class SparsePro(nn.Module):
    ...
    def __init__(self, P, K, XX, ytX, XtX, var_Y, h2, var_b):
        ...
        self.beta_prior_tau = torch.tile(torch.tensor(
            1.0 / var_b * np.array([k+1 for k in range(self.k)]), dtype=torch.float32), (self.p, 1))
    ...
    def forward(self, XX, ytX, XtX, LD):  # LD not used in this function
        ...
        gammaterm1 = (self.gamma * torch.tile(self.prior_pi.reshape(-1, 1),
                      (1, self.k))).sum()  # confirm torch.tile = np.tile
    ...
    def get_effect_num_dict(self):
        ...
        eff_gamma = {i: np.round(
            gamma[effect[i], i].tolist(), 8) for i in effect}
        eff_mu = {i: np.round(beta_mu[effect[i], i].tolist(), 8)
                  for i in effect}
...
for i in range(len(ldlists)):
    ...
    h2_hess, var_b = get_HESS_h2_SS(
        XtX, XX, LD.values, beta, se, args.N, args.var_Y)
    ...
    mcs = model.get_effect_dict() # model/most credible sets
    eff_gamma, eff_mu = model.get_effect_num_dict()

    pip_tensor = model.get_PIP()
    pip_vec = torch.round(pip_tensor, decimals=8)
